...
imagesc(Rule_tbl);
colorbar;
title('Таблиця правил');

% Похибки
figure;
bar(err);
set(gca,'XTickLabel',{'Gauss','Triangular','Trapezoid'});
title('Порівняння похибок');
ylabel('%');

%% ── Висновки ────────────────────────────────────────────────
fprintf('\n=== ВИСНОВКИ ===\n');
fprintf('Найкраща МФ: %d\n', find(err==min(err)));
fprintf('Найгірша МФ: %d\n', find(err==max(err)));